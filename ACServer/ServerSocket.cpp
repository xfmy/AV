...
int CServerSocket::init(int port)
{
	...
	if (err != 0)
	{
		MessageBox(NULL, L"初始化套接字库第一步失败", L"警告", MB_ICONWARNING);
		return err;
	}
	if (LOBYTE(wsaData.wVersion) != 1 || HIBYTE(wsaData.wVersion) != 1)
	{
		WSACleanup();
		MessageBox(NULL, L"初始化套接字库第二部失败", L"警告", MB_ICONWARNING);
		return-1;
	}
	//创建套接字
	...
	{
		MessageBox(NULL, L"创建套接字库失败", L"警告", MB_ICONWARNING);
		std::string str(100, 0);
		sprintf(const_cast<char*>(str.c_str()), "创建套接字库失败->%s", GetErrorInfo(WSAGetLastError()).c_str());
	...
	{
		MessageBox(NULL, L"监听到bind返回值为SOCKET_ERROR", L"警告", MB_ICONWARNING);
		std::string str(100, 0);
		sprintf(const_cast<char*>(str.c_str()), "bind失败->%s", GetErrorInfo(WSAGetLastError()).c_str());
		OutputDebugStringA(str.c_str());
		return -1;
	}
	return 0;
...
bool CServerSocket::start(int count)
{
	...
	{
		MessageBox(NULL, L"监听到listen返回值为SOCKET_ERROR", L"警告", MB_ICONWARNING);
		std::string str(100, 0);
		sprintf(const_cast<char*>(str.c_str()), "listen失败->%s", GetErrorInfo(WSAGetLastError()).c_str());
		OutputDebugStringA(str.c_str());
		return false;
	}
	this->count = count;
	...
	while (i++ < count)
	{
		...
		{
			MessageBox(NULL,L"检测到accept返回值为INVALID_SOCKET", L"警告", MB_ICONWARNING);
			std::string str(100, 0);
			sprintf(const_cast<char*>(str.c_str()), "accept失败->%s", GetErrorInfo(WSAGetLastError()).c_str());
			OutputDebugStringA(str.c_str());
			i--;
		}
		...
		std::string str(100,0);
		sprintf(const_cast<char*>(str.c_str()), "服务端接收成功 端口号 % d, ip == % d. % d. % d. % d\n", pt, *pIp, *(pIp + 1), *(pIp + 2), *(pIp + 3));
		OutputDebugStringA(str.c_str());
	}
...
BACK:
		...
		{
			std::string str;
			str = str + "recv失败->" + GetErrorInfo(WSAGetLastError()).c_str();
			OutputDebugStringA(str.c_str());
			return len;
		}
...
int CServerSocket::SendMes(DataBag& bag)
{
	...
	{
		std::string str;
		str = str + "send失败->" + GetErrorInfo(WSAGetLastError()).c_str();
		OutputDebugStringA(str.c_str());
	}
	return res;
}
